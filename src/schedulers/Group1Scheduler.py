...
class Group1Scheduler(StrategyScheduler):
    ...
    def fit(self, data_set, max_time):
        ...
        # precalculate statistics that are used for calculating the scheduling order
    ...
    def set_problem_and_features(self, problem_file, features):
        ...
        if self._standardize:
            x = pp.standardizeFeaturesWithMeansStds(x, self._means, self._stds, True, self._stdcap)
        x = pp.addPCAFeatures(x, self._pca_V, self._pcas)
    ...
    def __processRegressionVector(self, Yt, yo):
        ...
        # if it is a hard problem (yo) and regression value is below threshold set it to threshold
        if self._use_optimizer:
        ...
        # get time from regression prediction and multiply it so that it overestimates a little
        ...
        # if self._use_optimizer and not yo and t > 10: # original with t > 10 opt10diff without opt10
        # if it is an easy problem and time is higher than threshold set to threshold
        if self._use_optimizer:
    ...
    def predict(self, time_left):
        ...
        # calculate COVERAGE VECTOR - percentage of problems covered by the current strategies
        yc = np.sum(self._cover, axis=0) / float(self._cover.shape[0])
        # calculate PROB VECTOR - probs that a strategy within the strategy selector predicts the problem correctly given the pruned problems & predictions  
